...
const AnimatedItem = ({ children, delay = 0, index, onMouseEnter, onClick, gap = '1rem' }: AnimatedItemProps) => {
  ...
  const itemDelay = delay || Math.min(index * 0.03, 0.4);
  return (
    <motion.div
      ref={ref}
      data-index={index}
      ...
      transition={{ duration: 0.22, delay: itemDelay }}
...
type AnimatedListProps<T> = {
  ...
  /** Render arbitrary content per item instead of the default plain-text card. */
  ...
  /** Gap below each item, passed to the entrance-animation wrapper. Default '1rem'. */
  itemGap?: string;
};
